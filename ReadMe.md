...
1. **Email us your preferred Trello board name** to [info@agiledigitalworker.com](mailto:info@agiledigitalworker.com).
2. **Connect your repository:** Follow the 3-question card onboarding flow documented in the [User Guide — First-time setup](https://github.com/grandua/digitalworker-docs/blob/main/user-guide.md#first-time-setup): answer the three onboarding questions on your board's first card (repository HTTPS clone URL, branch to clone, and a narrowly scoped GitHub personal access token with repo read/write access), then delete the token comment once DigitalWorker confirms it.
3. **Add one bounded task** with clear acceptance criteria.
